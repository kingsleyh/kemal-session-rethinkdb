...
describe Kemal::Session::RethinkDBEngine do
  describe ".int" do
  ...
  describe ".destroy" do
    it "should remove session from rethinkDB" do
      session = Kemal::Session.new(create_context(SESSION_ID))
  ...
  describe "#destroy" do
    ...
    it "should succeed if session doesnt exist in rethinkDB" do
      session = Kemal::Session.new(create_context(SESSION_ID))
      value = r.table("sessions").filter({session_id: SESSION_ID}).run(Conn).to_a.size
      value.should eq(1)
  ...
  describe "#destroy_all" do
    it "should remove all sessions in rethinkDB" do
